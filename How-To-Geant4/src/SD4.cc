...
std::map<std::string, int> SD4map;

// initialize particleCounter outside of anything so this will work with multithreading
// G4int particle_counterSD4 = 0;

//Get process Main ID
std::thread::id main_idSD4 = std::this_thread::get_id();
...
SD4::~SD4()
{
	...
	if (main_idSD4 == thread_id)
	{
		// Get an iterator pointing to the first element in the map
		std::map<std::string, int>::iterator it_console = SD4map.begin();
		std::map<std::string, int>::iterator it_file = SD4map.begin();

		// Iterate through the map and print the elements in console
		G4cout << "PARTICLE COUNT OF SD4" << G4endl;
		while (it_console != SD4map.end())
		{
		...
		while(std::ifstream(folderName + "/" + ListFolder + "/" + std::to_string(pid) + "_ListOfGeneratedParticles in SD4" + ".txt"))
		{
			pid++;
		}
		// Set final file name
		std::string fileName = std::to_string(pid) + "_ListOfGeneratedParticles in SD4" + ".txt";

		// flush output to file
		std::ofstream outFile(folderName + "/" + ListFolder + "/" + fileName);

		// Iterate through the map and print the elements in file
		outFile <<  "PARTICLE COUNT OF SD4" << G4endl;
		while (it_file != SD4map.end())
		{
...
G4bool SD4::ProcessHits(G4Step* step, G4TouchableHistory* /*history*/)
{
	...
	if(current_name == "N14")  analysisManager->FillNtupleDColumn(4, 0, Ekin/MeV);
	...
	if(current_name == "N14")  analysisManager->AddNtupleRow(4);
