...
std::map<std::string, int> SD3map;

// initialize particleCounter outside of anything so this will work with multithreading
// G4int particle_counterSD3 = 0;

//Get process Main ID
std::thread::id main_idSD3 = std::this_thread::get_id();
...
SD3::~SD3()
{
	...
	if (main_idSD3 == thread_id)
	{
		...
		std::map<std::string, int>::iterator it_console = SD3map.begin();
		std::map<std::string, int>::iterator it_file = SD3map.begin();

		// Iterate through the map and print the elements in console
		G4cout << "PARTICLE COUNT OF SD3" << G4endl;
		while (it_console != SD3map.end())
		{
		...
		while(std::ifstream(folderName + "/" + ListFolder + "/" + std::to_string(pid) + "_ListOfGeneratedParticles in SD3" + ".txt"))
		{
			pid++;
		}
		// Set final file name
		std::string fileName = std::to_string(pid) + "_ListOfGeneratedParticles in SD3" + ".txt";

		// flush output to file
		std::ofstream outFile(folderName + "/" + ListFolder + "/" + fileName);

		// Iterate through the map and print the elements in file
		outFile <<  "PARTICLE COUNT OF SD3" << G4endl;
		while (it_file != SD3map.end())
		{
...
G4bool SD3::ProcessHits(G4Step* step, G4TouchableHistory* /*history*/)
{
	...
	if(current_name == "N14")  analysisManager->FillNtupleDColumn(3, 0, Ekin/MeV);
	...
	if(current_name == "N14")  analysisManager->AddNtupleRow(3);
