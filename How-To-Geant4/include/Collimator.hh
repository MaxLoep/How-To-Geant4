...
G4LogicalVolume* collimator(std::string name, std::map<std::string, double>& params, Materials::MaterialMaker mat) {
	...
	double b = params.count("b")? params["b"] : 5.*cm;   //Entrance_Diameter of the tungsten colli; MAX 6.14cm
	double c = params.count("c")? params["c"] : 5.*cm;   //inner diameter (choke) of the tungsten colli; MAX 6.14cm
	double d = params.count("d")? params["d"] : 5.*cm;   //Exit_Diameter of the tungsten colli; MAX 6.4cm
